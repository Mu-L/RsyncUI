...
| **F12** ad-hoc actor allocation on the read side | 🟡 Partial | The shared reader now exists, but `LogStoreService.loadStore` still creates `ActorReadLogRecords()` per call and `CreateOutputforView.createaoutputlogfileforview()` still creates `ActorLogToFile()` per read. |
