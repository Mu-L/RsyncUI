...
| **F4** constructor-as-side-effect on read/model side | 🟡 Partial | `ReadImportConfigurationsJSON` is now an explicit async `read(...)` API, but `UserConfiguration(importeddata)` and `_ = Snapshotlogsandcatalogs(...)` still rely on constructor side effects. |
