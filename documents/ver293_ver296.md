...
| **F12** ad-hoc actor allocation on the read side | 🟡 Partial | `ActorLogToFile` is now a singleton (`static let shared`, `private init()`), so logfile reads, writes, resets, and `logOutput(...)` calls from `Execute`, `Estimate`, `SshKeys`, `ObservableSchedules`, `InterruptProcess`, `LogfileView`, and `CreateOutputforView` all serialize against the same actor instance. `LogStoreService.loadStore` still creates `ActorReadLogRecords()` per call, but its actual decode is already routed through `SharedJSONStorageReader.shared`, so the wrapper adds no isolation value — collapsing it into a free async helper is cleanup, not correctness. |
...
**What is done:** the shared reader infrastructure now exists, and `ActorLogToFile` was promoted to a `static let shared` singleton so logfile reads/writes/resets serialize against one actor instance app-wide instead of being interleaved across throwaway instances.

**What remains:** `LogStoreService.loadStore` still allocates `ActorReadLogRecords()` per call (the real decode is already on `SharedJSONStorageReader.shared`, so this is a wrapper that can be collapsed into a free async helper); cross-profile task loading is still serial; version-info lookup still pays for duplicate remote fetches.

### Subsystem summary

| Subsystem | Status at HEAD |
|---|---|
| A. Persistence / storage | **F3**, **F4**, and **F6** closed; **F12** partially remains |
...
1. **F12 + F15** — reuse the shared reader/cached version info so the new infrastructure pays off in the remaining callers.
2. **F13** — parallelize cross-profile reads with structured concurrency.
3. **F8 + F14** — deduplicate regex parsing in the log domain.
4. **F11 + F10** — mechanical readability cleanup after the larger follow-ups.
